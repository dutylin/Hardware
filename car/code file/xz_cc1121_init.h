#ifndef  __XZ_CC1121_INIY_H_
#define  __XZ_CC1121_INIY_H_

#include "xz_spi.h"
#include "xz_cc1121_config.h"
...
typedef  struct {
  bool  rf_receive_end_flag;
  uint8 rf_receive_data_len;
  uint8 rf_receive_data_buff[20];  
  uint8 rf_pkt_rssi;
}CC1121_RF_RX_DATA;
...
typedef  union {                                  /*T&H DATA*/
  struct {
    uint32  pkt_id;
    uint8   pkt_tmp_integer;
    uint8   pkt_tmp_decimal;  
    uint8   pkt_hum_integer;
    uint8   pkt_hum_decimal;  
    uint8   pkt_status;
    uint8   pkt_voltage;
    uint8   pkt_serial_num;  
    uint8   pkt_rssi;
  }pkt_def;
  uint8  pkt_data_buff[20];
}DATA_PACKET_DEF;
#pragma pack()


extern CC1121_RF_RX_DATA RF_RECEIVE_DATA;
extern DATA_PACKET_DEF  Data_Packet;
