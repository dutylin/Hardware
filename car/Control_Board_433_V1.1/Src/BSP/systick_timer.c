...
各模块定时器延时
*/
unsigned int * get_systick_timer_value()
{     
      unsigned int pbuff[6];
	  
	pbuff[0]=GPS_Time_Interval;
	pbuff[1]=RF_Time_Interval;
...
void reconfig_systick_timer_value(uint32_t *pbuff)
{     
     // unsigned int pbuff[6];
	  
	GPS_Time_Interval=pbuff[0];
	RF_Time_Interval=pbuff[1];
	LTE_Delay_Time=pbuff[2];   
	LTE_Heart_Time=pbuff[3];
	LTE_Power_Time=pbuff[4];
	LTE_Battery_Time=pbuff[5];	      
