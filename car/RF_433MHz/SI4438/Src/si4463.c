...
SI443X_RF_DATA_DEF RF_RECEIVE_DATA;
DATA_PACKET_DEF  Data_Packet;
SI443X_RF_txDATA_DEF  Data_Packet_Tx;
// payload length
...
void SI4463_init(void)
{
	...
	spi_write(16, app_command_buf); 					// ????¨ª¡¤??
...
void fifo_cfg(U8 smode)
{
	...
	app_command_buf[14]  = 0x00;
	app_command_buf[15]  = 0x00;
	spi_write(16, app_command_buf);				// ¨¦¨¨?? Field 1--43¡è?¨¨
}
...
void spi_write_fifo(void)
{
	...
	for (i = 0; i< Data_Packet_Tx.rf_tx_data_len; i++)
	{
		spi_byte(Data_Packet_Tx.rf_tx_data_buff[i]);
	}
	nCS_SET();
	delay_us(1);
}
...
//	rx_len = 	spi_readbyte();  //¶Ápayload³¤¶È
...
//	for (i = 0; i< rx_len; i++)
//	{
//		rx_ph_data[rf_rx_index++] = spi_readbyte(); //¶ÁpayloadÊý¾Ý
...
void fifo_reset(void)			// ?¡ä??¡¤¡é¨¦?o¨ª?¨®¨º? FIFO
{
	U8 p[2];
	p[0] = FIFO_INFO;
...
//unsigned char RF_CC1121_ReadRSSI()
//{
//	spi_read(9,PART_INFO);
...
//	unsigned int crc16_res;
	...
	if(RF_RECEIVE_DATA.rf_receive_end_flag==true)
	{
		...
		if((spi_read_buf[4] &0x08) == 0)	// crc error check
		{
			...
			for(i=0; i<rx_len; i++)
			{
				read_data=spi_readbyte();  //¶Ápayload³¤¶ÈCC1121_ReadReg(CC112x_FIFO,false);
				RF_RECEIVE_DATA.rf_receive_data_buff[i]=read_data;
			}
			RF_RECEIVE_DATA.rf_receive_data_len=rx_len;
			rx_init();
			fifo_reset();
		...
		Data_Packet.pkt_def.pkt_rssi=1;//RF_CC1121_ReadRSSI();
