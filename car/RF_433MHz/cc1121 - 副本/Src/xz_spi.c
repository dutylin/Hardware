...
*/
 ...
 uint8 UTIL_SPI_3W_ReadWriteUint8(uint8 data)   //移位顺序为高BIT最先移出
{
  uint8 i;  //这是使用uint8,作用是在不同的平台下，以最快的变量类型进行循环运算
  uint8 temp;
 ...
 uint8 UTIL_SPI_3W_ReadWriteUint8(uint8 data)   //移位顺序为低BIT最先移出
{
  uint8 i;  //这是使用uint8,作用是在不同的平台下，以最快的变量类型进行循环运算
  uint8 temp;
...
void  BOARD_CC1121_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA,RF_SPI_NSS, GPIO_PIN_SET);
  HAL_GPIO_WritePin(GPIOA, RF_SPI_MOSI, GPIO_PIN_SET);	
