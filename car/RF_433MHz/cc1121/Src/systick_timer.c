...
unsigned int   RF_Delay_Time =12000;
...
uint32_t Time_delay2[systick_count];//采样、延时时间设置
void Sample_timer_inc()
{
	unsigned char i;
	for(i=0; i<systick_count; i++)
	{
		if(Time_delay[i]>Time_delay2[i])
			Time_delay2[i]=Time_delay2[i]+1;
	}
...
void Sample_Timer_Reload()
{
	...
	for(i=0; i<systick_count; i++)
	{
		if(Time_delay2[i]==Time_delay[i]&&Time_delay2[i]!=0)
		{
			switch(i)
			{
			case 0:
			{
				Time_delay2[i] = 0;Time_delay[i] = 0;
				//if(timer_enable==1)
