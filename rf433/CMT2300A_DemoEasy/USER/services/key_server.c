    ...
    switch(pKey->status)
    {
        case KEY_STATE_IDLE:
        {
            if(!keyFlag) {
                ...
                pKey->timestamp = g_nSysTickCount;
        ...
        case KEY_STATE_FIRST_DOWN:
        {
            ...
            else if(g_nSysTickCount - pKey->timestamp >= KEY_JITTER_TIME) {
...
BOOL Key_IsLongDown(tKeyScan* pKey, u32 timeStep)
{
    ...
    if(KEY_STATE_DOWN==pKey->status && (g_nSysTickCount-pKey->timestamp) > timeStep) {
        ret = TRUE;
        pKey->timestamp = g_nSysTickCount;
    }
