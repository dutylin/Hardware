...
/******************************************************************************
º¯ÊýÃû£ºRTC_GetTimeDate
º¯Êý¹¦ÄÜ£º¶ÁÈ¡rtcÊ±¼äºÍÈÕÆÚ
******************************************************************************/
...
void SYS_RTCInit(RTC_SetTypeDef RTC_SetStructure)
{
  ...
  {        
	;  //³õÊ¼»¯Ê±ÖÓÊ§°Ü,¾§ÕñÓÐÎÊÌâ£¬32768¾§ÕñÆðÕñ¼«ÏÞÊ±¼ä1-5S     
  }   
...
{    
  ...
  RTC_ClearFlag(RTC_FLAG_WUTF);	      
	...
	//Ñ¡ÔñÊ±ÖÓÎªÍâ²¿32.768KHz 8·ÖÆµ    
	RTC_SetWakeUpCounter(5);         //¼ä¸ô500ms
	RTC_ClearITPendingBit(RTC_IT_WUT);	
	RTC_ITConfig(RTC_IT_WUT,ENABLE);    //ÐèÒªÊ¹ÄÜÖÐ¶Ï£¬²»ÐèÒªÖÐ¶Ïº¯Ê
	RTC_WakeUpCmd(ENABLE);
 
	//    

}
void RTC_IRQDISABLE(void);
...
{    
	...
	NVIC_Init(&NVIC_InitStructure);
      // RTC_ClearITPendingBit(RTC_IT_WUT);    
	//RTC_ITConfig(RTC_IT_WUT,ENABLE);    //ÐèÒªÊ¹ÄÜÖÐ¶Ï£¬²»ÐèÒªÖÐ¶Ïº¯Êý
...
{    
	...
	NVIC_Init(&NVIC_InitStructure);
       RTC_ClearITPendingBit(RTC_IT_WUT);    
	RTC_ITConfig(RTC_IT_WUT,DISABLE);    //ÐèÒªÊ¹ÄÜÖÐ¶Ï£¬²»ÐèÒªÖÐ¶Ïº¯Êý
	  RTC_ClearITPendingBit(RTC_IT_WUT);	
}
void RTC_WKUP_IRQHandler(void)
{
    ...
    if(RTC_GetITStatus(RTC_IT_WUT) != RESET) 
    {
        ...
        EXTI_ClearITPendingBit(EXTI_Line20); 
        PWR_RTCAccessCmd(ENABLE);//Èç¹ûÖ®Ç°½ûÖ¹ÁË£¬ÒªÖØÐÂ´ò¿ª
        RTC_ClearITPendingBit(RTC_IT_WUT);
        PWR_RTCAccessCmd(DISABLE);//¿ÉÒÔÔÙ¹ØÉÏ
       // RTC_ClearITPendingBit(RTC_IT_WUT);   //Çå³ýÖÐ¶Ï±êÖ¾
    }
