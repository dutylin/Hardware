...
void W25QXX_CS(u8 bitoperation)
{
	if(bitoperation == 0)
	{
...
******************************************************************************/
	...
	unsigned char M25PXX_BUFFER[M25PXX_SECTOR_BYTES_SIZE];

void M25PXX_Write(unsigned char* pBuffer,uint32_t WriteAddr,uint32_t NumByteToWrite)
{
	uint32_t secpos;
	uint32_t secoff;
	uint32_t secremain;
	...
	while(1)
	{
		...
		{
			/*if(M25PXX_BUF[secoff+i]!=0XFF)
			{
				secoff=secoff+M25PXX_BUF[0]<<24+M25PXX_BUF[1]<<16+M25PXX_BUF[2]<<8+M25PXX_BUF[3];
				break;//需要擦除
			}*/
			if(M25PXX_BUF[secoff+i]!=0XFF)
			{
		...
		if(i<secremain)//需要擦除
		{
			...
			M25PXX_Write_NoCheck(M25PXX_BUF,secpos*M25PXX_SECTOR_BYTES_SIZE,M25PXX_SECTOR_BYTES_SIZE);//写入整个扇区
...
uint32_t M25PXX_ReadID(void)
{
	...
	unsigned char test_rx[1024];
    unsigned char test_tx[1024]; 
...
{
 	M25PXX_Write((unsigned char*)pbuff,WriteAddr,len); 	//从倒数第100个地址处开始,写入SIZE长度的数据
	return 1;
}
...
{
	//M25PXX_Read(pbuff,Raddress,len);

	if(M25PXX_Read((unsigned char*)pbuff,Raddress,len)==1) 	//从倒数第100个地址处开始,写入SIZE长    
	{
...
unsigned  char M25PXX_Erase_Head()
{
  unsigned char head_len_value[4]={0xff,0xff,0xff,0xff};
  M25PXX_Write(head_len_value,0x0000,4); 	//从倒数第100个地址处开始,写入SIZE长度的数据
	return 0;
}
