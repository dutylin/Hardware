...
uint8_t TxCounter = 0;
u8  USART1_RX_BUF[USART1_MAX_RECV_LEN]; 		//接收缓冲,最大USART3_MAX_RECV_LEN字节
u8  USART1_TX_BUF[USART1_MAX_SEND_LEN]; 		//发送缓冲,最大USART3_MAX_SEND_LEN字节
u16 USART1_RX_STA;
u8 rtc_flag;
u8   uart1_it_tx_flag;
u8  uart_receive_index;
...
INTERRUPT_HANDLER(DMA1_CHANNEL0_1_IRQHandler,2)
{
  ...
  */
  ...
  PotVoltage =(uint32_t)((float)12.250*1024/ (uint32_t)Buffer[0]) ;
...
//    {
//      
//      USART1_RX_BUF[USART1_RX_STA++]=temp;  //记录接收到的值
//      
//    }
//    else
//    {
...
INTERRUPT_HANDLER(USART1_RX_TIM5_CC_IRQHandler, 28)
{
  ...
  */
  ...
  /* Read one byte from the receive data register and send it back */
  temp = (USART_ReceiveData8(EVAL_COM1) & 0xFF);
  USART_ClearFlag(EVAL_COM1, USART_IT_RXNE);
  uart_receive_index=uart_receive_index+1;
  //temp = (USART_ReceiveData8(EVAL_COM1) & 0x7F);
  if((USART1_RX_STA&0x8000)==0)                //接收完的一批数据,还没有被处理,则不再接收其他数据
  { 
    if(USART1_RX_STA<USART1_MAX_RECV_LEN)     //还可以接收数据
