...
dev_data.len=dev_data.len+2;
//pump status
dev_data.payload[index_tmp++]=get_pump_status();
dev_data.len=dev_data.len+1;
//crc code
...
void server_command_process()
{
  ...
  if(index!=255)
  {
    ...
    if(dev_sn==server_cmd.sn||server_cmd.sn==0xffff)
    {
      ...
      {  
        //control pum on or off
        if( server_cmd.cmd_num==PUMP_ON||((m_Tem>=sensor_params.TEMPERATURE_MAX)&&(m_Hum<=sensor_params.HUM_MIN)))
        {GPIO_ToggleBits(GPIOD,GPIO_Pin_7);
        PUMP_CTRL(1);	  
        }
        if( server_cmd.cmd_num==PUMP_OFF||((m_Tem<=sensor_params.TEMPERATURE_MIN)||(m_Hum>=sensor_params.HUM_MAX)))
        {
          GPIO_ToggleBits(GPIOD,GPIO_Pin_7);	   
          //PUMP_CTRL(2);
          PUMP_CTRL(0);
        ...
        //setting temperature and hum by server
        if( server_cmd.cmd_num==SENSOR_TEMPERATURE_HUM_SET)
        {
          sensor_params.TEMPERATURE_MAX=USART1_RX_BUF[index+6]*256+USART1_RX_BUF[index+1+6];
          sensor_params.TEMPERATURE_MIN=USART1_RX_BUF[index+2+6]*256+USART1_RX_BUF[index+3+6];
          if(USART1_RX_BUF[index+4+6]!=0||USART1_RX_BUF[index+5+6]!=0)
          {
            sensor_params.HUM_MAX=USART1_RX_BUF[index+4+6]*256+USART1_RX_BUF[index+5+6];
            sensor_params.HUM_MIN=USART1_RX_BUF[index+6+6]*256+USART1_RX_BUF[index+7+6];	
          }
  ...
  if(((m_Tem>=sensor_params.TEMPERATURE_MAX)&&(m_Hum<=sensor_params.HUM_MIN)))
  {
    PUMP_CTRL(1);	  
  }
  if( ((m_Tem<=sensor_params.TEMPERATURE_MIN)||(m_Hum>=sensor_params.HUM_MAX)))
  {
    GPIO_ToggleBits(GPIOD,GPIO_Pin_7);	   
    PUMP_CTRL(0);
  }
...
void sensor_init()
{
  sensor_params.TEMPERATURE_MAX=400;
  sensor_params.TEMPERATURE_MIN=250;
  sensor_params.HUM_MAX=600;
  sensor_params.HUM_MIN=400;
}
