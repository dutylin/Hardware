	 ...
	 if(UsartType3.receive_flag==1)
	  {
		...
		if((UsartType3.usartDMA_rxBuf[UsartType3.real_index]*256+UsartType3.usartDMA_rxBuf[UsartType3.real_index+1])==SER_HEAD)
		{
			if(CRC_ENABLE == 1)
			{
...
void RF_Process()
{
  ...
  if(Lora_Node_process())
	{
   uart_tx(3,UsartType3.usartDMA_txBuf,UsartType3.tx_len);
   UsartType3.tx_len=0;	
	}
...
/***************************************************************************/			
	...
	if((UsartType2.usartDMA_rxBuf[UsartType2.real_index]*256+UsartType2.usartDMA_rxBuf[UsartType2.real_index+1])==DEV_HEAD)		
		{
			if(CRC_ENABLE == 1)
			{
...
unsigned char Lora_Node_process()
{
	unsigned char res;
	res=0;
	if(Lora_Node_PayloadProecss())
	 {
	       memcpy(&UsartType3.usartDMA_txBuf[0],&UsartType2.usartDMA_rxBuf[UsartType2.real_index],UsartType2.rx_len_var);
        UsartType3.tx_len=UsartType3.tx_len+UsartType2.rx_len_var;
